...
async function apiRequest(path, options = {}, token = null) {
  ...
  if (!response.ok) {
    throw new Error(data?.detail || 'Ocurrió un error al comunicarse con el servidor.');
  }
...
export default function App() {
  ...
  const adjustAge = (amount) => {
    let currentAge = parseInt(currentUser.edad, 10) || 18;
  ...
  const startTest = (e) => {
    ...
    if (!age || age < 1 || age > 120 || !currentUser.carrera) {
      setError('Por favor, captura una edad válida y el acrónimo de carrera.');
      return;
    }
    setError('');
    setCurrentView('test');
  ...
  const renderResults = () => {
    const record = lastRecord || { score: 0, edad: currentUser.edad, carrera: currentUser.carrera, interpretation: getInterpretation(0) };
    ...
    return (
      <div className="max-w-md mx-auto bg-white p-8 sm:p-10 rounded-[2rem] shadow-xl shadow-slate-200/50 border border-slate-100 text-center relative overflow-hidden">
        ...
        <div className="grid grid-cols-2 gap-3 mb-10 text-left">
          ...
          <div className="bg-slate-50 p-4 rounded-2xl border border-slate-100"><div className="text-xs text-slate-400 font-bold uppercase tracking-wider mb-1">Carrera</div><div className="text-slate-800 font-semibold">{record.carrera}</div></div>
        </div>
  ...
  const renderLogin = () => (
    <div className="max-w-md mx-auto bg-white p-8 sm:p-10 rounded-[2rem] shadow-xl shadow-slate-200/50 border border-slate-100">
      <div className="flex items-center gap-3 mb-8"><div className="bg-slate-900 p-3 rounded-xl text-white"><LogIn size={24} /></div><div><h2 className="text-3xl font-extrabold text-slate-900">Acceso admin</h2><p className="text-slate-500 text-sm">Inicia sesión para ver datos y estadísticas.</p></div></div>
      {error && <ErrorBox message={error} />}
    ...
    return (
      <div className="max-w-6xl mx-auto bg-white p-6 sm:p-10 rounded-[2rem] shadow-xl shadow-slate-200/50 border border-slate-100">
        ...
        <section className="grid grid-cols-1 lg:grid-cols-3 gap-4 mb-8">
          <MetricCard icon={<Database size={24} />} label="Total agrupado" value={statsSummary.total} />
          <MetricCard icon={<BarChart3 size={24} />} label="Promedio ponderado" value={statsSummary.average.toFixed(1)} />
          <MetricCard icon={<CalendarDays size={24} />} label="Agrupación" value={statsPeriod === 'day' ? 'Día' : statsPeriod === 'month' ? 'Mes' : 'Año'} />
        </section>
        ...
        <div className="grid grid-cols-1 xl:grid-cols-2 gap-8">
          <DataTable title="Estadísticas" empty="No hay estadísticas para mostrar." headers={['Periodo', 'N', 'Promedio', 'Mín', 'Máx']} rows={stats.map(s => [s.period, s.count, s.average_score.toFixed(2), s.min_score, s.max_score])} />
          <DataTable title="Registros recientes" empty="Aún no hay participantes registrados." headers={['Fecha', 'Edad', 'Carrera', 'Puntaje', 'Nivel']} rows={records.map(r => [new Date(r.created_at).toLocaleString(), r.edad, r.carrera, r.score, r.interpretation])} />
        </div>
      </div>
    );
  };
